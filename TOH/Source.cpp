...
void strokeString(float x, float y, float sx, float sy, string s, int width)
{
	int i;
	glLineWidth(width);
	glPushMatrix();
...
void first()
{
	...
	glColor3f(1, 0, 0);
	strokeString(50, 850, 0.15, 0.15, "University Visvesvaraya College of Engineering", 2);
	...
	glColor3f(1, 1, 0);
	strokeString(650, 160, 0.18, 0.18, "Shubham N J", 2);
	strokeString(650, 60, 0.15, 0.18, "Rachana Shetty", 2);
	glColor3f(1, 0, 0);
	strokeString(650, 120, 0.18, 0.18, "19GACSE057", 2);
	strokeString(650, 30, 0.2, 0.18, "19GACSE045", 2);
	glutSwapBuffers();
}
